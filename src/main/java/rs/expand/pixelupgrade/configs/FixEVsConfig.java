...
public class FixEVsConfig
{
    ...
    private Path configPath = Paths.get(path, "FixEVs.conf");
    ...
    public void loadOrCreateConfig(Path configPath, ConfigurationLoader<CommentedConfigurationNode> configLoader)
    {
        ...
        if (Files.notExists(configPath))
        {
            try
            {
                ...
                Asset asset = Sponge.getAssetManager().getAsset(PixelUpgrade.getInstance(), "FixEVs.conf").get();
        ...
        else
        {
            ...
            {
                PixelUpgrade.log.info("\u00A74Error during config loading for command \"/fixevs\"!");
