// The party version of /timedhatch, broken off due to the insane complexity of cramming that all into a single class.
...
public class PartyHatch implements CommandExecutor
{
    ...
    public CommandResult execute(final CommandSource src, final CommandContext args)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (!src.hasPermission("empc.command.bypass.partyhatch") && cooldownMap.containsKey(playerUUID))
                {
                    final boolean hasAltPerm = src.hasPermission("empc.command.altcooldown.partyhatch");
                ...
                if (arg1Optional.isPresent())
                {
                    ...
                    else if (src.hasPermission("empc.command.other.partyhatch"))
                    {
            ...
            else
            {
                ...
                if (!getPartyHasEggs(party))
                {
                    sendCheckedMessage(src,"§4Error: §cNo eggs were found.");
                    return CommandResult.empty();
                }
                ...
                if (economyEnabled && !calledRemotely && commandCost > 0)
                {
                    ...
                    if (commandConfirmed)
                    {
                        ...
                        if (optionalAccount.isPresent())
                        {
                            ...
                            if (transactionResult.getResult() == ResultType.SUCCESS)
                            {
                                ...
                                {
                                    printSourcedMessage(sourceName, "§bPlayer §3" + player.getName() + " §bis hatching §3" +
                    ...
                    else
                    {
                        ...
                        if (target == null)
                        {
                            ...
                            {
                                sendCheckedMessage(src,"§6Warning: §eHatching your team costs §6" + costToConfirm +
                                        "§e coins.");
                            }
    ...
    // Check if at least one egg is present in the party that's being checked.
    // TODO: Maybe merge this with hatchParty. We run through two very similar loops if a party does contain eggs.
    private boolean getPartyHasEggs(final PartyStorage storage)
    {
        // Create some variables to fill in as we go. Remember: slots start from zero!
        Pokemon pokemon;
        boolean hatchedEgg = false;
        for (int i = 0; i <= 5; i++)
        {
